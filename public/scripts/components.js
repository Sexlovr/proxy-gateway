...
export function toast(message, type = 'info', duration = 4000) {
  ...
  const el = h('div', { class: `toast ${type}`, html: `${icons[type] || icons.info}<div class="toast-msg">${esc(message)}</div><button class="toast-x" aria-label="Dismiss"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>` });
...
export function openModal(content, opts = {}) {
  ...
  const inner = h('div', { class: `content glass ${size}` },
    closable && h('button', { class: 'close', 'aria-label': 'Close', html: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>' }),
    content,
  );
